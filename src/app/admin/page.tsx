    ...
    >
      <div className="min-w-0">
        ...
        <p className="mt-1 text-xs text-[var(--color-text-subtle)]">
          {post.categoryName || '미분류'} · {formatKoreanReadableDate(post.createdAt)}
        </p>
      </div>
    ...
    <>
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 truncate text-xs text-[var(--color-text-subtle)]">
          {getAdminCommentAuthor(comment)} · {comment.postTitle || '게시글 정보 없음'} · {formatKoreanReadableDate(comment.createdAt, true)}
        </p>
      </div>
...
export default function AdminPage() {
  ...
  const { data: latestData, isLoading: isLatestLoading } = useQuery({
    queryKey: queryKeys.posts.adminLatest,
    queryFn: () => getPosts({ size: 5, sort: 'createdAt,desc' }),
    retry: 0,
  });

  const { data: popularData, isLoading: isPopularLoading } = useQuery({
    queryKey: queryKeys.posts.adminPopular,
  ...
  const { data: commentsData, isLoading: isCommentsLoading } = useQuery({
    queryKey: queryKeys.comments.adminRecent,
  ...
  const totalPosts = dashboard?.overview.totalPosts ?? getPageMeta(latestData).totalElements;
  const totalCategories = dashboard?.overview.totalCategories ?? countCategories(categories);
  const totalComments = dashboard?.overview.totalComments ?? getPageMeta(commentsData).totalElements;
      ...
      <header className="flex flex-col justify-between gap-5 border-b border-[var(--color-line)] pb-8 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-3 inline-flex items-center gap-2 text-xs font-medium text-[var(--color-text-subtle)]">
            ...
            오늘 {formatKoreanReadableDate(new Date())}
