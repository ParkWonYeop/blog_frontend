...
import { createCategory, deleteCategory, getCategories, updateCategory } from '@/features/category/api';
import WindowSurface from '@/shared/ui/WindowSurface';
import { flattenCategories, sortCategoriesById } from '@/features/category/lib';
import type { FlatCategory } from '@/features/category/lib';
import { getPrefixedErrorMessage } from '@/shared/lib/errors';
import { queryKeys } from '@/shared/lib/queryKeys';
import type { Category, CategoryUpdateRequest } from '@/shared/types';
...
export default function AdminCategoryPanel() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => toast.error(getPrefixedErrorMessage(error, '카테고리 생성 실패')),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => toast.error(getPrefixedErrorMessage(error, '카테고리 수정 실패')),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => toast.error(getPrefixedErrorMessage(error, '카테고리 삭제 실패')),
  });
